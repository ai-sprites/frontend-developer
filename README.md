...
```bash
git clone https://github.com/ai-sprites/frontend-developer.git
...
```bash
npx --yes --package=github:ai-sprites/frontend-developer#v0.1.0 sprite-frontend-developer init ./my-project --tool codex
```
...
角色说明位于 [templates/agent.md](templates/agent.md)。实际宿主的发现机制和验证边界见 [工具适配说明](docs/tool-support.md)。在 Claude Code 可用 `claude --agent sprite-frontend-developer`；Copilot CLI 可用 `copilot --agent=sprite-frontend-developer`；Codex 的原生角色调用与当前安装版本有关；本轮未验证成功，可按工具适配说明显式读取角色指令。
...
```bash
node bin/cli.js init ../my-project --tool codex --skills-only --practices sprite-frontend-developer-ui
```

`--practices all` 选择本包全部实践；逗号列表选择部分。首次 `--skills-only` 不产生角色，之后可以运行普通 `init` 补装。省略实践选项保留已装内容，不解释为删除。

```bash
node bin/cli.js update ../my-project --practices sprite-frontend-developer-ui
node bin/cli.js remove ../my-project --practices sprite-frontend-developer-ui
```

实践按当前任务提供方法和本地参考，不自动覆盖项目已有约定。取消实践仅移除本包未被修改的生成文件。

## 生成内容与共存

| 工具 | 角色文件 | 已选实践 |
| --- | --- | --- |
| Codex | `.codex/agents/sprite-frontend-developer.toml` | `.agents/skills/<实践>/` |
| Claude | `.claude/agents/sprite-frontend-developer.md` | `.claude/skills/<实践>/` |
| Copilot | `.github/agents/sprite-frontend-developer.agent.md` | `.github/skills/<实践>/` |

仅生成所选工具的文件，以及 `.sprite/installations/frontend-developer.<工具>.json` 中本包的文件归属与版本。不会修改项目的 README、AGENTS.md、CLAUDE.md、应用配置或业务文档。多个角色可按任意顺序安装；重复安装未修改的组件保持原样。
